  ...
  <div>
    ...
    <div class="page" v-if="!loading && sharedLists != null">
      <div class="panel">
        ...
        <select class="custom-select" v-model="activeList" :disabled="testRunning" @change="getWords()">
          <option :value="list" :key="list.id" v-for="list in sharedLists">{{list.title}}</option>
        </select>
        ...
        <div class="radios">
          <div class="custom-control custom-radio custom-control-inline">
            ...
            <label class="custom-control-label labellang" for="radio1">{{activeList.lang1}}</label>
          </div>
          <div class="custom-control custom-radio custom-control-inline">
            <input type="radio" id="radio2" name="answerLang" class="custom-control-input" value="2" v-model="answerLang" :disabled="testRunning">
            <label class="custom-control-label labellang" for="radio2">{{activeList.lang2}}</label>
          </div>
        </div>
        ...
        <transition name="fade">
          <div class="scoreboard" v-if="scoreboard != null">
            ...
            <ul class="list-group list-group-flush">
              <li class="list-group-item score" :key="score.id" v-for="(score, index) in scoreboard">
                <p><b>{{index+1}}. {{score.nick}}</b></p>
                <p>{{score.correct}}/{{score.allCount}}.0</p>
                <p v-if = "score.attempts >= 5">{{score.attempts}} podejść</p>
                <p v-if = "score.attempts >= 2 && score.attempts <= 4">{{score.attempts}} podejścia</p>
...
export default {
  ...
  methods: {
    ...
    getScoreboard(){
      if(this.activeList != null){
        this.$http.get('scoreboard/'+this.activeList.listId).then(response => {
          this.scoreboard = response.body;
          this.scoreboard.sort((a,b) => {
            return b.correct/b.allCount - a.correct/a.allCount;
          })
    ...
    setRemaining(){
      return "height: "+(1-this.remaining/this.result.all)*100+"% ; width: 100%";
    },
